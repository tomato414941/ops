...
export function SessionView({ sessionId, initialMessages }: SessionViewProps) {
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
      {error && (
        <div className="mx-4 mt-4 p-4 bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-300 rounded-lg">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 p-4">
        {messages.map((msg) => (
          <div
            ...
            className={`p-3 rounded-lg ${
              msg.role === "user"
                ? "bg-blue-100 dark:bg-blue-900/50 ml-8"
        ...
        {streamOutput && (
          <div className="p-3 rounded-lg bg-gray-100 dark:bg-gray-700 mr-8">
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Claude</div>
            <div className="whitespace-pre-wrap text-gray-900 dark:text-gray-100">{streamOutput}</div>
          </div>
        )}
        {isLoading && !streamOutput && (
          <div className="p-3 rounded-lg bg-gray-100 dark:bg-gray-700 mr-8">
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Claude</div>
            <div className="text-gray-400 dark:text-gray-500">thinking...</div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex gap-2">
          <input
            ...
            disabled={isLoading}
            className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
          />
          <button
            type="submit"
